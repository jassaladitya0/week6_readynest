...
import { UploadCloud, FileText, Database, CheckCircle2, AlertTriangle, Cpu, Layers } from 'lucide-react';
import Papa from 'papaparse';
import * as XLSX from 'xlsx';
import { SAMPLE_DATASETS } from '../utils/sampleData';
import { getColumnTypes } from '../utils/dataProcessor';

export default function DataIngestView({ data, datasetInfo, onDataLoaded, onSelectPreset }) {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      {/* Hero Header */}
      <div className="glass-panel" style={{ padding: '2rem', background: 'linear-gradient(135deg, rgba(30, 41, 59, 0.9) 0%, rgba(15, 23, 42, 0.9) 100%)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.5rem' }}>
            <span className="badge badge-blue">Data Ingestion Suite</span>
            <span className="badge badge-emerald">Ready</span>
          </div>
          <h2 className="title-gradient" style={{ fontSize: '1.8rem' }}>Multi-Source Ingestion & Schema Profiler</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '0.3rem' }}>
            Upload custom CSV, Excel, or JSON datasets, or analyze preloaded enterprise benchmarks.
          </p>
        </div>
        ...
        >
          <UploadCloud size={36} color="#60a5fa" style={{ marginBottom: '0.5rem' }} />
          <div style={{ fontSize: '0.9rem', fontWeight: 700, color: 'white' }}>Click or Drag File Here</div>
          <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)', marginTop: '0.2rem' }}>CSV, XLSX, XLS, JSON</div>
          <input 
            type="file" 
            ref={fileInputRef} 
      ...
      <div>
        <h3 style={{ fontSize: '1.1rem', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Database size={18} color="#c084fc" /> Preset Enterprise Datasets
        </h3>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: '0.5rem', fontSize: '0.75rem', color: 'var(--text-dim)' }}>
                  <span>📊 {preset.data.length} Records</span>
                  <span>•</span>
                  <span>📑 {Object.keys(preset.data[0] || {}).length} Columns</span>
                </div>
              </div>
            );
          })}
      ...
      {/* Schema Profiler Table */}
      <div className="glass-panel" style={{ padding: '1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h3 style={{ fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Layers size={18} color="#34d399" /> Column Schema & Data Type Profiler
          </h3>
          <div style={{ display: 'flex', gap: '1rem', fontSize: '0.825rem', color: 'var(--text-muted)' }}>
            <span>Total Rows: <strong style={{ color: 'white' }}>{totalRows}</strong></span>
            <span>Total Columns: <strong style={{ color: 'white' }}>{colCount}</strong></span>
          </div>
        </div>
